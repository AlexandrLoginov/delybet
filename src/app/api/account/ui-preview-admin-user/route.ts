...
import { UI_PREVIEW_ADMIN_USER_COOKIE } from "@/lib/ui-preview-admin-user-cookie";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = NextResponse.json({ ok: true as const, enabled });

    res.cookies.set({
      name: UI_PREVIEW_ADMIN_USER_COOKIE,
      value: enabled ? "1" : "",
      httpOnly: true,
